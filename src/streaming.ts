/**
 * Vendored from lab-cosmo/chemiscope@v1.1.0 (app/streaming.ts), BSD-3-Clause
 * (https://github.com/lab-cosmo/chemiscope). Self-contained: the type-only
 * import from ../src/dataset was removed; types stay as annotations.
 * Exports: loadDatasetStreaming, shouldUseStreaming, progressMessage,
 * parseJsonWithNaN, releaseActiveStream.
 */
/**
...
class StructureStore {
  ...
  public static async open(): Promise<StructureStore> {
    ...
    const db = await new Promise<IDBDatabase>((resolve, reject) => {
      ...
      req.onerror = () =>
        reject(req.error ?? new Error("failed to open browser storage"));
    });
    ...
    try {
      ...
      const stale = dbs
        ...
        .filter((name) => {
          ...
          const ts = Number(
            name.slice(StructureStore.NAME_PREFIX.length).split("-")[0],
          );
          return (
            Number.isFinite(ts) && now - ts > StructureStore.ORPHAN_MAX_AGE_MS
          );
        });
...
export async function loadDatasetStreaming(
  ...
  let byteStream: ReadableStream<Uint8Array> = file
    .stream()
  ...
  async function parseProperties(): Promise<Record<string, Property>> {
    ...
    while (true) {
      ...
      properties[name] = parseJsonWithNaN(
        await parser.captureValue(),
      ) as Property;
      reportProgress();
    }
    return properties;
