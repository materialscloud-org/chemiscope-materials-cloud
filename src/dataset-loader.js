...
import { formatSize, setLoadingText, setProgress, showSpinner, uiTick } from './progress.js';
import {
...
export async function loadDataset(url) {
    ...
    if (contentLength === 0) {
        ...
        if (buffer.byteLength === 0 && url.startsWith('https://cors.materialscloud.org/')) {
            throw Error("unable to load file at '" + url + "'");
        }
...
async function loadFile(file) {
    if (shouldUseStreaming(file)) {
        ...
        setProgress(null);
        const { dataset, loadStructure } = await loadDatasetStreaming(file, function (progress) {
            setLoadingText(
