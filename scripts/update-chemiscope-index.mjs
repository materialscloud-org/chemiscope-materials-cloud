...
function recordMetadata(metadata) {
  ...
  return {
    ...
    license:
      (metadata.rights && metadata.rights[0] && metadata.rights[0].title && metadata.rights[0].title.en) || '',
    subjects: (metadata.subjects || []).map((subject) => subject.subject).filter(Boolean),
  };
}
...
export async function fetchChemiscopeDatasets() {
  ...
  while (true) {
    ...
    for (const hit of data.hits.hits) {
      ...
      const files = chemiscopeFiles(entries);
      if (files.length > 0) {
        records.push({
          id: hit.id,
    ...
    console.log(`  page ${page}: ${data.hits.total} total hits, ${records.length} records so far`);
